            ...
            <div class="panel-body">
                <div class="col-md-12">
                    ...
                    <p class="animated fadeInDown">
                        ...
                        Room ID: {{$data['id']}}<span class="fa-angle-right fa"></span><br/>
                            ...
                            <table id="datatables-example" class="table table-striped table-bordered"
                                ...
                                <form action="" method="post">
                                    @forelse($search as $sea)
                                        <tr>
                                            <td>{{$sea->id}}</td>
                                            <td><input type="hidden" name="bookingRooms_room_id"
                                                       value="{{$sea->booking_id}}"> {{$sea->Room_id}}</td>
                                            <td><input type="hidden" name="bookingRooms_itinerary"
                                                       value="{{$sea->itinerary}}">{{$sea->itinerary}}</td>
                                            <td><input type="hidden" name="bookingRooms_start_date"
                                                       value="{{$sea->start_date}}">{{$sea->start_date}}</td>
                                            <td><input type="hidden" name="bookingRooms_end_date"
                                                       value="{{$sea->end_date}}">{{$sea->end_date}}</td>
                                            <td><input type="hidden" name="bookingRoom_no_of_people"
